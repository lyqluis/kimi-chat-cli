...
export const HistoryList = forwardRef<HistoryListHandle, HistoryViewProps>(
	({ viewMode, setViewMode, setChatId }, ref) => {
		// states
		...
		const cachedNormalHistory = useRef<HistoryResponse | null>(null)
		...
		const loadHistory = async (
			...
			try {
				// const res = await fetchMockChatHistory(payload)
		...
		const deleteHistory = async (index: number) => {
			...
			try {
				...
				if (res.chatId) {
					// ENHANCE?: 保存删除前的数据长度，用于重新加载相同数量的数据
		...
		const handleSubmit = async (value: string) => {
			...
			// 提交搜索前缓存当前正常数据
			cachedNormalHistory.current = historyData
			loadHistory({ page_size: 10, query }, true)
		...
		useImperativeHandle(ref, () => ({
			...
			cancelSearch: () => {
				// use normal data when query is empty and quit search mode
				if (!query) {
					setHistoryData(cachedNormalHistory.current)
				}
				setIsSearchMode(false)
			},
		}))
		...
		useInput(
			(inputStr, key) => {
				...
				// select mode
				if (!isSearchMode && historyData && historyData.chats.length > 0) {
					// 上下箭头选择 (如果有数据)
					...
					if (key.downArrow || inputStr === "j") {
						...
						const canScrollDown = () => {
							...
							return (
								scrollOffset < bottomOffset && contentHeight > viewportHeight
							)
						}
			...
			<Box flexDirection="column" flexGrow={1} height={rows - 1}>
				...
				<Box marginTop={1} paddingX={1}>
					...
					<Text color="gray">
						...
						<Text bold>Space</Text>: expand to preview, <Text bold>M</Text>:
						load more, <Text bold>D</Text>: delete chat
					</Text>
				</Box>
			</Box>
		)
